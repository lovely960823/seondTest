			...
			<div class="col-md-12" style="width: 1380px;">
				<table class="table table-hover table-condensed table-bordered">
					<thead>
						<tr>
							<td colspan="9">
							<form class="form-inline" action="classList" id="classListSearchFrm">
								<!-- 默认看第一页 ,简化点击分页链接时候带表单参数问题-->
 								<input name="pageNo" type="hidden" value="1">
									...
									<input type="button" class="btn btn-info" value="我的选择" onclick="myselect()" />&nbsp;&nbsp;&nbsp;
									<input type="button" class="btn btn-info" value="修改密码" onclick="toPwdEditStudent()" />&nbsp;&nbsp;&nbsp;
					...
					<tbody>
						...
						<tr>
							...
							<td >${tclass.count}</td>
							<td >
							<a href="" onclick="sel(${tclass.id },${tclass.tid },${tclass.locationId },${tclass.timeId})">
									选择
								</a>
							</td>
						</tr>
						...
						<tr>
							...
							<nav aria-label="Page navigation">
									...
									<c:otherwise>
										<li class="page-item">
										<a href="classList?pageNo=${pageInfo.number+1-1}" class="page-link">&laquo;</a>
										</li>
									</c:otherwise>
									</c:choose>
									...
									<c:choose>
										<c:when test="${pageInfo.totalPages<=5}">
											<c:forEach begin="1" end="${pageInfo.totalPages}" var="pi">
												<li class="page-item">
												<a href="classList?pageNo=${pi}" class="page-link">${pi}</a>
												</li>
											</c:forEach>
										</c:when>
										<c:when test="${pageInfo.number+1<=3}">
											<li class="page-item">
											<a href="classList?pageNo=1" class="page-link">1</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=2" class="page-link">2</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=3" class="page-link">3</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=4" class="page-link">4</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.totalPages}" class="page-link">...${pageInfo.totalPages}</a>
											</li>
										</c:when>
										<c:when test="${pageInfo.number+1>=pageInfo.totalPages-2}">
											...
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.totalPages-3}" class="page-link">${pageInfo.totalPages-3}</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.totalPages-2}" class="page-link">${pageInfo.totalPages-2}</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.totalPages-1}" class="page-link">${pageInfo.totalPages-1}</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.totalPages}" class="page-link">${pageInfo.totalPages}</a>
											</li>
										...
										<c:otherwise>
											...
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.number+1-1}" class="page-link">${pageInfo.number+1-1}</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.number+1}" class="page-link">${pageInfo.number+1}</a>
											</li>
											<li class="page-item">
											<a href="classList?pageNo=${pageInfo.number+1+1}" class="page-link">${pageInfo.number+1+1}</a>
											</li>
									...
									<c:otherwise>
										<li class="page-item">
										<a href="classList?pageNo=${pageInfo.number+1+1}" class="page-link">&raquo;</a>
										</li>
									</c:otherwise>
									</c:choose>
									</ul>
...
function sel(id,tid,locationId,timeId){
	if (confirm("确认选择？")) {
		 $.ajax({ 
		url:"selectClass",
		data:{'id':id,
...
function detail(tid){
	...
	function toPwdEditStudent(){
		 window.location.href="toPwdEditStudent" 
	}
	</script>
<script type="text/javascript">
		$(function() {
			$("a[ href='classList?pageNo=${pageInfo.number+1}']").parent("li").addClass("active");
			//让Tid回填
			$('#selTid').val("${empty param.tid?-1:param.tid}");
			//让超级链接追加表单参数，最简单方式阻止超级链接默认行为，为表单添加隐藏字段a['class="page-link"']
			$('a[class="page-link"][href^="classList?pageNo="]').click(function(evt){
				evt.preventDefault();
				var prePage=${pageInfo.number+1-1};
				var nextPage=${pageInfo.number+1+1};
				//修改表单中pageNo的值
				if($(this).text()=="«"){
					$('#classListSearchFrm input[name="pageNo"]').val(prePage);
				}else if($(this).text()=="»"){
					$('#classListSearchFrm input[name="pageNo"]').val(nextPage);
				}else{
					$('#classListSearchFrm input[name="pageNo"]').val($(this).text().replace(/\./g,''));
				}
				$("#classListSearchFrm").submit();
			});
		});
