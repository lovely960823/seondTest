...
<div class="container-fluid">
		<div class="row">
			<div class="col-md-12">
				...
				<div style="height: 50px;padding-left: 550px"><h1>学生列表</h1></div>
				...
				<c:choose>
				  <c:when test="${empty pageInfo.content}">
					<tr>
					<td colspan="9" align="center">暂无无数据</td>
					</tr>
					</c:when>
				   <c:otherwise>
						<c:forEach items="${pageInfo.content}" var="student" varStatus="l">
						<tr>
							<td >${l.count}</td>
							<td  class="btn1" onclick="detail('${student.id}')" ><a href="javascript:void(0)">${student.name}</a> </td>
							<td >${student.num}</td>
							<td ><fmt:formatDate value="${student.pubdate}" pattern="yyyy-MM-dd"/></td>
							<td >${student.phone}</td>
							...
							<td >
							     ...
							     &nbsp;&nbsp;&nbsp;&nbsp;
								 <a href="toStudentEdit?id=${student.id}"  >
									<img alt="" src="resources/img/edit.png" title="修改">
							   </a>
								&nbsp;&nbsp;&nbsp;&nbsp;
							   <a href="" onclick="del('${student.id }')">
								    <img alt="" src="resources/img/del.png" title="删除">
								</a>
							   &nbsp;&nbsp;
								<a href="" onclick="ret('${student.id }')">
						...
						<tr >
							...
							<nav aria-label="Page navigation">
									...
									<c:otherwise>
										<li class="page-item">
										<a href="toStudentList?pageNo=${pageInfo.number+1-1}" class="page-link">&laquo;</a>
										</li>
									</c:otherwise>
									</c:choose>
									...
									<c:choose>
										<c:when test="${pageInfo.totalPages<=5}">
											<c:forEach begin="1" end="${pageInfo.totalPages}" var="pi">
												<li class="page-item">
												<a href="toStudentList?pageNo=${pi}" class="page-link">${pi}</a>
												</li>
											</c:forEach>
										</c:when>
										<c:when test="${pageInfo.number+1<=3}">
											<li class="page-item">
											<a href="toStudentList?pageNo=1" class="page-link">1</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=2" class="page-link">2</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=3" class="page-link">3</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=4" class="page-link">4</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.totalPages}" class="page-link">...${pageInfo.totalPages}</a>
											</li>
										</c:when>
										<c:when test="${pageInfo.number+1>=pageInfo.totalPages-2}">
											...
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.totalPages-3}" class="page-link">${pageInfo.totalPages-3}</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.totalPages-2}" class="page-link">${pageInfo.totalPages-2}</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.totalPages-1}" class="page-link">${pageInfo.totalPages-1}</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.totalPages}" class="page-link">${pageInfo.totalPages}</a>
											</li>
										...
										<c:otherwise>
											...
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.number+1-1}" class="page-link">${pageInfo.number+1-1}</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.number+1}" class="page-link">${pageInfo.number+1}</a>
											</li>
											<li class="page-item">
											<a href="toStudentList?pageNo=${pageInfo.number+1+1}" class="page-link">${pageInfo.number+1+1}</a>
											</li>
									...
									<c:otherwise>
										<li class="page-item">
										<a href="toStudentList?pageNo=${pageInfo.number+1+1}" class="page-link">&raquo;</a>
										</li>
									</c:otherwise>
									</c:choose>
									</ul>
